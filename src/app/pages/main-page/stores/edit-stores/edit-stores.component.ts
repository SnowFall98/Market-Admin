...
import { Istores } from 'src/app/interface/istroes';
...
export class EditStoresComponent implements OnInit {
  ...
  constructor(private form: FormBuilder, private storeService: StoresService,
    private imageService: ImagesService, private http: HttpClient,
    public dialogRef: MatDialogRef<EditStoresComponent>, 
    @Inject(MAT_DIALOG_DATA) public data: IDialogData) { }

  ngOnInit(): void {

    this.storeService.getItem(this.data.id)
    .subscribe(
      ...
      (resp:any) =>{
        ...
        this.dialCode = resp.phone.splice("-")[0];
  ...
  editStore(){
    ...
    if(this.uploadLogo){
      ...
      this.imageService.uploadImage(this.uploadLogo, "stores", this.urlView, 300, 300, this.nameLogo)
      .subscribe((resp:any)=>{
    ...
    if(this.uploadCover){
      ...
      this.imageService.uploadImage(this.uploadCover, "stores", this.urlView, 1500, 800, this.nameCover)
      .subscribe((resp:any)=>{
        ...
        if(resp.status == 200 ){
          ...
          alerts.basicAlert("Error", 'Invalid Cover', "error")
        }
      })
    }
    ...
    this.storeService.patchData(this.data.id, dataSource).subscribe(
